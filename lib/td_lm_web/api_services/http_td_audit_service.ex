defmodule TdLmWeb.ApiServices.HttpTdAuditService do
  @moduledoc false
  ...
  def post_audits(%{"audit" => _audit_params} = req) do
    ...
    case HTTPoison.post(get_audits_path(), body, headers, []) do
      ...
      {:ok, _response = %HTTPoison.Response{status_code: 422}} ->
        Logger.error(
          "Error 422 in audit service (maybe Redis service is down): post_audits function"
        )
  ...
  defp get_config do
    Application.get_env(:td_lm, :audit_service)
  end

  defp get_audit_endpoint do
    audit_service_config = get_config()

    "#{audit_service_config[:protocol]}://#{audit_service_config[:audit_host]}:#{
      audit_service_config[:audit_port]
    }"
  end
